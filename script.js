class MedicationReminder {
    ...
    async requestNotificationPermission() {
        if ('Notification' in window) {
            if (Notification.permission === 'default') {
                ...
                banner.innerHTML = `
                    <span class="banner-text">${this.getCurrentTranslation().enableNotifications}</span>
                    <button class="btn-enable" onclick="medicationApp.enableNotifications()">${this.getCurrentTranslation().enableNotifications}</button>
                `;
    ...
    updateLanguage() {
        ...
        document.querySelector('.add-medication h2').textContent = t.addNew;
        document.querySelector('label[for="medName"]').textContent = t.medName;
        document.querySelector('label[for="medDosage"]').textContent = t.dosage;
        document.querySelector('label[for="medDate"]').textContent = t.date;
        document.querySelector('label[for="medTime"]').textContent = t.time;
        document.querySelector('.btn-primary').innerHTML = `<i class="fas fa-plus"></i> ${t.addMed}`;
        document.querySelector('.medications-list h2').textContent = t.medList;
        
        // Update filter tabs
        const tabs = document.querySelectorAll('.tab-btn');
        tabs[0].textContent = t.all;
        tabs[1].textContent = t.upcoming;
        tabs[2].textContent = t.past;
...
setInterval(() => {
    medicationApp.scheduleNotifications();
}, 60000);